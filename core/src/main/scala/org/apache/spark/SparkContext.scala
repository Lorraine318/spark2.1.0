...
/**
  ...
  */
//sparkDriver的初始化始终围绕着SparkContext的初始化
class SparkContext(config: SparkConf) extends Logging {

  // 其中保存着线程栈中最靠近栈顶的用户定义的类及最靠近栈底的scala或者spark核心类信息。
  ...
  private var _conf: SparkConf = _ //sparkContext的配置，在SparkContext初始化的过程中会对conf中的配置信息做校验，例如必须设置spark.master等
  private var _eventLogDir: Option[URI] = None //事件日志的路径，当spark.eventLog.enabled属性为true时启用。 默认是/tmp/spark-events      也可以通过spark.eventLog.dir属性来指定
  private var _eventLogCodec: Option[String] = None //事件日志的压缩算法，当spark.eventLog.enable和spark.eventLog.compress使用都为true启用。默认压缩lz4,也可以通过spark.io.compression.codec属性来指定  目前支持的压缩算法：lzf,snappy,lz4
  private var _listenerBus: LiveListenerBus = _ //sparkContext中的事件总线
  private var _env: SparkEnv = _ //spark运行环境
  private var _statusTracker: SparkStatusTracker = _ //提供对作业，Stage等的监控信息
  private var _progressBar: Option[ConsoleProgressBar] = None //利用sparkStatusTracker的API，在控制台展示Stage的进度。
  private var _ui: Option[SparkUI] = None //spark的用户界面
  private var _hadoopConfiguration: Configuration = _ //hadoop的配置西悉尼
  private var _executorMemory: Int = _ //executor的内存大侠，默认值为1024MB.可以通过：SPAKR_MEM 或者SPARK_EXECUTOR_MEMORY 或者 spark.executor.memory属性来指定  spark.executor.memory > SPARK_EXECUTOR_MEMORY > SPARK_MEM 优先级
  private var _schedulerBackend: SchedulerBackend = _ //创建taskScheduler时候，会将SchedulerBcked和taskScheduler的对偶返回，这里的_schedulerBacked就是那的SchedulerBacked的引用
  private var _taskScheduler: TaskScheduler = _ //任务调度器 创建taskScheduler时候返回的value,k 是上面——schedulerBackend
  private var _heartbeatReceiver: RpcEndpointRef = _ //心跳接收器，所有executor都会向heartbeatReceiver发送心跳信息，heartbeatReceiver接收到Executor的心跳信息后，首先更新Executor的最后可见时间，然后将此信息交给taskScheduler做进一步处理
  @volatile private var _dagScheduler: DAGScheduler = _ //DAG调度器 ，负责创建job,将DAG中的RDD划分到不同的stage，提交stage等
  private var _applicationId: String = _ //当前应用的标识  taskScheduler启动后会创建应用标识。
  private var _applicationAttemptId: Option[String] = None //当前应用尝试执行的标识
  private var _eventLogger: Option[EventLoggingListener] = None //将时间持久化到存储的监听器，当spark.eventLog.enabled属性为true时启用。
  private var _executorAllocationManager: Option[ExecutorAllocationManager] = None //executor动态分配管理器，可以根据工作负载动态调整executor的数量，在配置spark.dynamicAllocation.enabled属性为true的前提下。在非local模式下或者当spark.dynamicAllocation.testing属性为true时启用
  private var _cleaner: Option[ContextCleaner] = None //上下文清理器    实际用异步方式清理那些超出应用作用域范围的rdd,broadcast,shufferDependency等信息
  private var _listenerBusStarted: Boolean = false //liveListenerBus是否已经启动的标记
  private var _jars: Seq[String] = _ //用户设置的jar文件，当用户选择的部署模式是yarn,_jars由spark.jars和spark.yarn.dist.jars的并集,如果是其他模式，只采用spark.jars即可
  private var _files: Seq[String] = _ //用户设置的文件，通过spark.files属性来指定
  private var _shutdownHookRef: AnyRef = _ //shutdownHookManager  用于设置关闭钩子的管理器没这样就可以在JVM进程退出时，执行一些清理工作
  private var _statusStore: AppStatusStore = _ //AppStatusStore封装了 监听器AppStatusListener，最后会注册到监听器列表中
  ...
  // 用来保存每个本地文件的URL与添加此文件到addFiles时的时间之间的映射缓存
  private[spark] val addedFiles = new ConcurrentHashMap[String, Long]().asScala
  //用来保存每个本地jar文件的URL与添加此文件到addedJars时的时间戳的映射缓存
  ...
  // 用于存储环境变量。 exevutorEnvs中环境变量都将传递给执行任务的executor使用
  private[spark] val executorEnvs = HashMap[String, String]()

  // 当前系统的登录用户，也可以通过SPARK_USER进行配置
  ...
  try {
    ...
    // 将appstatusListenerBus监听器添加到liveListenerBus监听器列表。
    _statusStore = AppStatusStore.createLiveStore(conf)
    listenerBus.addToStatusQueue(_statusStore.listener.get)

    //通过创建SparkEnv的构造器的时候，将liveListenerBus监听器列表传递给SparkEnv和SparkEnv的内部组件中。
    ...
    //创建Spark状态跟踪器 SparkStatusTracker
    ...
    //调用的WebUI的bind方法来启动spark UI底层的Jetty服务
    _ui.foreach(_.bind())

    _hadoopConfiguration = SparkHadoopUtil.get.newConfiguration(_conf)

    // 在addJar中会将jar添加到Driver的RPC环境中
    if (jars != null) {
      jars.foreach(addJar)
    }
    ...
    //向HeartbeatReceiver发送taskSchedulerIsSet消息，表示_taskScheduler已经持有了TaskScheduler的引用
    ...
    //初始化sparkEnv 中的BlockManager
    _env.blockManager.initialize(_applicationId)

    //metrics-System是SparkEnv内部的组件之一
    _env.metricsSystem.start()
    //  attachHandler将度量系统的ServletContextHandler添加到了SparkUI中
    _env.metricsSystem.getServletHandlers.foreach(handler => ui.foreach(_.attachHandler(handler)))

    //事件日志监听器
    _eventLogger =
      //当spark.eventLog.enabled 为true时启用
      if (isEventLogEnabled) {
        val logger =
    ...
    // 判断是否需要启用Executor-AllocationManager
    val dynamicAllocationEnabled = Utils.isDynamicAllocationEnabled(_conf)
    //是基于工作负载动态分配和删除Executor的代理
    _executorAllocationManager =
      if (dynamicAllocationEnabled) {
        schedulerBackend match { //当创建TaskScheduler时的schedulerBackend同时实现了ExecutorAllocationClient才开始开始创建ExecutorAllocationManager
    ...
    //清理超出应用范围的RDD，shuffle元数据，broadCast对象， rdd的 checkpoint等数据
    _cleaner =
      //通过spark.cleaner.referenceTracking的配置来决定是否启用ContextCleaner
    ...
    //向事件总线投递SparkListenerApplicationStart事件,即应用启动事件
    postApplicationStart()

    // 等待SchedulerBackend准备完成
    _taskScheduler.postStartHook()
    //向度量系统注册DAGSchedulerSource,BlockManagerSource, ExecutorAllocationManagerSource
    ...
    //添加SparkContext的关闭钩子,使JVM退出之前调用SparkContext的stop方法进行一些关闭工作。
  ...
  /**
    ...
    */
  //用于向LiveListenerBus中添加实现了特质SparkListenerInterface的监听器
  @DeveloperApi
  ...
  /**
    ...
    * @note A path can be added only once. Subsequent additions of the same path are ignored.
    */
  //将jar添加到Driver本地RpcEnv的nettyStreamManager中
  ...
  private def setupAndStartListenerBus(): Unit = {
    ...
    //启动事件总线，并将_listenerBusStarted事件总线的标记置为启动
  ...
  private def postEnvironmentUpdate() {
    if (taskScheduler != null) {
      ...
      val environmentDetails = SparkEnv.environmentDetails(conf, schedulingMode, addedJarPaths,
        addedFilePaths)
      //生成SparkListenerEnvironmentUpdate事件
      val environmentUpdate = SparkListenerEnvironmentUpdate(environmentDetails)
      //将事件提交到事件总线上，此事件最终被EnvironmentListener监听，并印象EnvironmentPage页面的输出结果
      listenerBus.post(environmentUpdate)
    }
  }

  // 为了防止同时激活多个SparkContext，请将其标记为上下文已完成建设。
  //备注：必须将其放置在SparkContext构造函数的末尾。
  //将SparkContext标记为激活，setActiveContext方法
...
object SparkContext extends Logging {
  ...
  //对SparkContext进行构建的时候使用的锁，以此保证SparkContext的构建过程是线程安全的
 ...
 //用于保存激活的SparkContext
  ...
  //负责请求集群管理器给应用程序分配并运行Executor（一级调度） 和 给任务分配Executor并运行任务（二级调度）
  private def createTaskScheduler(
    ...
    //不同的部署模式，创建的taksScheduler的方式也不一样
    master match {
      case "local" =>
